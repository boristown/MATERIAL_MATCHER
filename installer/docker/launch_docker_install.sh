#!/usr/bin/env bash
# Docker 方式安装入口：终端执行 ./启动Docker安装.sh（CLI-first）。
set -u
cd "$(dirname "$0")" || exit 1
if [[ ! -f docker_wizard.sh ]]; then
  echo "缺少 docker_wizard.sh：01-Docker方式 目录不完整，请重新完整复制。" >&2
  exit 1
fi
...
echo "需要 root 或 sudo 管理员权限。请用管理员账号运行：sudo ./启动Docker安装.sh" >&2
exit 44
