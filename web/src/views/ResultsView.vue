...
function evaluate(row: Row): void { router.push(`/tasks/${row.id}/evaluation`) }
onMounted(load)
</script>
...
<template>
  <div class="results-page">
    <div class="toolbar">
      <div><h2>STEP 4 · 输出结果</h2><p>已完成任务:生成匹配摘要与样式化 Excel。</p></div>
    ...
    <div class="panel">
      ...
      <el-table :data="rows" size="default">
        ...
        <el-table-column label="操作" min-width="170"><template #default="scope">
          ...
          <el-button v-if="scope.row.status==='COMPLETED'" link type="success" @click="evaluate(scope.row)">准确率验收</el-button>
