...
class SessionStore:
    ...
    TTL 由 MATERIAL_MATCHER_SESSION_TTL_SECONDS 配置，并直接作为服务端会话寿命；
    调用方必须使用同一 ttl_seconds 设置 Cookie Max-Age，避免浏览器与服务端不一致。
    授权状态不依赖会话缓存——中间件每次请求都从 users 表核对 enabled/role。
    """
