...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    async def auth_middleware(request: Request, call_next: Any) -> Response:
        ...
        public_paths = {"/api/health", "/api/health/ready", "/api/auth/login", "/api/docs"}
    ...
    def init_upload(payload: UploadInit) -> dict[str, object]:
        ...
        temporary_path = cfg.data_dir / "tmp" / f"upload-{upload_id}{suffix}.part"
        with metadata.connect() as connection:
    ...
    async def upload_chunk(upload_id: str, index: int, request: Request) -> dict[str, int]:
        ...
        if row is None:
            raise DomainError("UPLOAD_NOT_FOUND", "分块上传任务不存在", status_code=404)
        data = await request.body()
        if index != row["next_chunk"]:
            raise DomainError("UPLOAD_CHUNK_ORDER", "分块顺序不正确", status_code=409, details={"next_chunk": row["next_chunk"]})
        if len(data) > cfg.chunk_size_bytes:
            raise DomainError("UPLOAD_CHUNK_TOO_LARGE", "单个分块超过允许大小", status_code=413)
        received_bytes = int(row["received_bytes"]) + len(data)
        if received_bytes > int(row["expected_size"]):
            raise DomainError("UPLOAD_SIZE_MISMATCH", "上传数据超过预期文件大小", status_code=409)
        path = Path(str(row["tmp_path"]))
        path.parent.mkdir(parents=True, exist_ok=True)
        with path.open("ab") as output:
            output.write(data)
        with metadata.connect() as connection:
            connection.execute("UPDATE upload_sessions SET received_bytes=?, next_chunk=? WHERE upload_id=?", (received_bytes, index + 1, upload_id))
    ...
    def complete_upload(upload_id: str) -> dict[str, object]:
        ...
        if row["received_bytes"] != row["expected_size"]:
            raise DomainError("UPLOAD_SIZE_MISMATCH", "上传文件大小与预期不一致", status_code=409)
        if row["expected_sha256"] and _sha256_file(path).lower() != str(row["expected_sha256"]).lower():
            raise DomainError("UPLOAD_HASH_MISMATCH", "上传文件校验失败", status_code=409)
        with path.open("rb") as stream:
            record = files.save_stream(str(row["original_name"]), str(row["role"]), stream, cfg.max_total_upload_bytes)
    ...
    def create_catalog(payload: CatalogCreate) -> dict[str, object]:
        ...
        if file_record["role"] != "target":
            raise DomainError("INVALID_FILE_ROLE", "集团码目录必须使用 target 文件", status_code=422)
        catalog_id = uuid.uuid4().hex
        version_id = uuid.uuid4().hex
        created_at = _now()
        with metadata.connect() as connection:
            connection.execute("INSERT INTO catalogs VALUES(?,?,?)", (catalog_id, payload.name, created_at))
            connection.execute("INSERT INTO catalog_versions VALUES(?,?,?,?,?,?,?)", (version_id, catalog_id, payload.source_file_id, payload.group_code_column, "READY", 1, created_at))
        return get_catalog_version(version_id)

    @app.get("/api/catalogs")
    def list_catalogs() -> list[dict[str, object]]:
        with metadata.connect() as connection:
            rows = connection.execute("SELECT c.name, v.* FROM catalog_versions v JOIN catalogs c ON c.catalog_id=v.catalog_id ORDER BY v.created_at DESC").fetchall()
    ...
    def create_draft(payload: DraftCreate) -> dict[str, object]:
        return tasks.create_draft(payload.name)

    @app.get("/api/task-drafts")
    def list_drafts() -> list[dict[str, object]]:
        return tasks.list_drafts()

    @app.get("/api/task-drafts/{draft_id}")
    def get_draft(draft_id: str) -> dict[str, object]:
        return tasks.get_draft(draft_id)
    ...
    def save_draft_data(draft_id: str, payload: DraftData) -> dict[str, object]:
        ...
        return tasks.save_data(draft_id, payload.model_dump())
    ...
    def get_progress(task_id: str) -> dict[str, object]:
        ...
        return {
            ...
            "steps": [
                {"key": "prepare", "label": "数据准备", "status": "WAITING"},
                {"key": "embedding", "label": "向量化处理", "status": "WAITING"},
                {"key": "retrieve", "label": "候选比对", "status": "WAITING"},
    ...
    def system_info() -> dict[str, object]:
        return {
            ...
            "database": str(metadata.db_path),
            "max_upload_bytes": cfg.max_upload_bytes,
            "max_total_upload_bytes": cfg.max_total_upload_bytes,
