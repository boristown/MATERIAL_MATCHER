...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    def ready() -> dict[str, object]:
        ...
        checks = {"admin_account": admin_count > 0, "database": os.access(metadata.db_path, os.R_OK | os.W_OK), "data_dir": os.access(cfg.data_dir, os.W_OK), "tmp_dir": os.access(cfg.data_dir / "tmp", os.W_OK)}
    ...
    @app.get("/api/auth/me")
    def auth_me(request: Request) -> dict[str, object]: return users.get(str(request.state.username))

    @app.post("/api/auth/change-password")
    def change_password(request: Request, payload: ChangePasswordRequest, response: Response) -> dict[str, object]:
        username = str(request.state.username); users.authenticate(username, payload.current_password); user = users.set_password(username, payload.new_password, must_change_password=False); sessions.revoke_user(username); response.delete_cookie(COOKIE_NAME, path="/"); return {"ok": True, "user": user, "relogin_required": True}

    @app.post("/api/auth/logout")
    def logout(request: Request, response: Response) -> dict[str, bool]: sessions.revoke(request.cookies.get(COOKIE_NAME)); response.delete_cookie(COOKIE_NAME, path="/"); return {"ok": True}
    ...
    def update_user(username: str, request: Request, payload: UserUpdate) -> dict[str, object]:
        ...
        result = users.update(username, role=payload.role, enabled=payload.enabled); sessions.revoke_user(username); return result
    ...
    def init_upload(payload: UploadInit) -> dict[str, object]:
        ...
        with metadata.connect() as connection: connection.execute("INSERT INTO upload_sessions VALUES(?,?,?,?,?,?,?,?,?)", (upload_id, payload.role, payload.original_name, payload.total_size, payload.sha256, 0, 0, str(temporary_path), _now()))
    ...
    def list_catalogs() -> list[dict[str, object]]: return catalogs.list_versions()
    @app.get("/api/catalogs/versions/{version_id}")
    def get_catalog(version_id: str) -> dict[str, object]: return catalogs.version(version_id)
    @app.get("/api/catalogs/{catalog_id}/versions")
    def catalog_versions(catalog_id: str) -> list[dict[str, object]]: return catalogs.versions(catalog_id)
    @app.post("/api/catalogs/{catalog_id}/versions")
    def create_catalog_version(catalog_id: str, payload: CatalogVersionCreate) -> dict[str, object]: return catalogs.add_version(catalog_id, source_file_id=payload.source_file_id, group_code_column=payload.group_code_column, activate=payload.activate)
    @app.post("/api/catalogs/{catalog_id}/versions/{version_id}/activate")
    def activate_catalog_version(catalog_id: str, version_id: str) -> dict[str, object]: return catalogs.activate(catalog_id, version_id)
    ...
    def rename_profile(profile_id: str, payload: ProfileRename) -> dict[str, object]: return profiles.rename(profile_id, payload.name)

    @app.delete("/api/profiles/{profile_id}")
    def delete_profile(profile_id: str, request: Request) -> dict[str, object]:
        if str(request.state.role) != "admin": raise DomainError("PERMISSION_DENIED", "仅管理员可删除匹配方案", status_code=403)
    ...
    def get_progress(task_id: str) -> dict[str, object]:
        ...
        return {"task_id":task_id,"stage":stage,"status":status,"progress":task["progress"],"processed_rows":task["processed_rows"],"total_rows":task["total_rows"],"error_code":task.get("error_code"),"error_message":task.get("error_message"),"execution_mode":mode,"current_phase":phase,"index_id":runtime.get("index_id"),"estimate":estimate,"last_progress_heartbeat":runtime.get("updated_at"),"worker_count":cfg.match_workers,"batch_size":cfg.match_batch_rows,"query_batch_size":cfg.query_batch_size,"started_at":task.get("started_at"),"compute_started_at":task.get("compute_started_at"),"compute_completed_at":task.get("compute_completed_at"),"compute_duration_ms":task.get("compute_duration_ms"),"compute_elapsed_ms":task.get("compute_elapsed_ms"),"live_counts":matches.live_counts(task_id),"interim":status not in {"COMPLETED","FAILED"},"steps":[{"key":"prepare","label":"数据准备","status":prepare},{"key":"embedding","label":"向量化 / 索引准备","status":embedding_status},{"key":"retrieve","label":"候选召回","status":retrieve_status},{"key":"rerank","label":"精细评分","status":rerank_status},{"key":"prepare_result","label":"结果持久化","status":result_status}]}
    ...
    def live_results(task_id: str, limit: int = Query(20, ge=1, le=200)) -> dict[str, object]:
        task = tasks.get_task(task_id)
        with metadata.connect() as connection: rows = connection.execute("SELECT source_id,current_status,top1_group_code,top1_score,second_score,score_gap,final_group_code,updated_at FROM match_items WHERE task_id=? ORDER BY rowid DESC LIMIT ?", (task_id, limit)).fetchall()
    ...
    def get_evaluation(run_id: str, include_items: bool = False, only_errors: bool = False, limit: int = Query(200, ge=1, le=2000)) -> dict[str, object]: return evaluations.get(run_id, include_items=include_items, only_errors=only_errors, limit=limit)
    @app.get("/api/indexes")
    def list_indexes(limit:int=Query(50,ge=1,le=200))->dict[str,object]: versions=matches.indexes.list_versions()[:limit]; return {"total":len(versions),"items":versions}
