...
class Settings:
    ...
    match_workers: int = 4
    match_batch_rows: int = 256
    progress_update_seconds: float = 1.0
    ...
    def load(cls) -> "Settings":
        return cls(
            ...
            match_workers=max(1, int(os.getenv("MATERIAL_MATCHER_MATCH_WORKERS", "4"))),
            match_batch_rows=max(1, int(os.getenv("MATERIAL_MATCHER_MATCH_BATCH_ROWS", "256"))),
            progress_update_seconds=max(0.1, float(os.getenv("MATERIAL_MATCHER_PROGRESS_UPDATE_SECONDS", "1.0"))),
            query_batch_size=max(1, int(os.getenv("MATERIAL_MATCHER_QUERY_BATCH_SIZE", "64"))),
            ann_min_rows=max(1, int(os.getenv("MATERIAL_MATCHER_ANN_MIN_ROWS", "100000"))),
            ann_lsh_tables=max(1, int(os.getenv("MATERIAL_MATCHER_ANN_LSH_TABLES", "48"))),
            ann_lsh_bits=max(4, min(24, int(os.getenv("MATERIAL_MATCHER_ANN_LSH_BITS", "16")))),
            ann_probe_radius=max(0, min(1, int(os.getenv("MATERIAL_MATCHER_ANN_PROBE_RADIUS", "1")))),
            ann_candidate_limit=max(128, int(os.getenv("MATERIAL_MATCHER_ANN_CANDIDATE_LIMIT", "12000"))),
