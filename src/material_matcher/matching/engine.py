...
def load_target_rows_with_position(path: Path, *, max_target_rows: int) -> list[tuple[int, dict[str, object]]]:
    ...
    rows = [(position, _to_plain(row)) for position, row in iter_tabular_rows_with_position(path)]
...
def _row_result(
    ...
    candidates = [
        CandidateResult(
            ...
            item[1],
            round(item[0], 4),
            float(item[3]["raw_score"]),
            bool(item[3]["critical_conflict"]),
            item[2],
            list(item[3]["field_scores"]),
            item[4],
        )
...
def match_rows(
    ...
    *,
    ...
    total = min(source_layout.row_count_estimate, max_source_rows) if max_source_rows is not None else source_layout.row_count_estimate
    ...
    for row_index, (source_row_number, source_raw) in enumerate(
        ...
        for target_row_number, target_row in target_rows:
            ...
            scored.append((score.display_score, str(code), target_row, score.to_dict(), target_row_number))
    ...
    def flush(items: list[tuple[int, int, dict[str, object]]]) -> None:
        ...
        for position, (row_index, source_row_number, source_row) in enumerate(items):
            ...
            for hit in hits:
                ...
                target_row_number = int(raw_target_row_number) if raw_target_row_number is not None else None
