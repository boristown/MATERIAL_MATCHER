...
class BenchmarkService:
    ...
    Embedding benchmark uses the configured production provider/model. The vector
    kernel benchmark intentionally uses deterministic embeddings so CI and offline
    preflight can measure the actual BBQ build/search implementation without
    pretending that it measures model inference throughput.
    """
    ...
    def runtime_status(self) -> dict[str, object]:
        return embedding_runtime_status(self.settings)
    ...
    def run_vector_kernel(
        ...
        *,
        ...
        try:
            ...
            cache = EmbeddingCache(root / "cache", provider)
            ...
            index, stats = EmbeddedBBQFlatIndex.build(
                ...
                metadata={"benchmark": True, "benchmark_kind": "synthetic_vector_kernel"},
                embedding_batch_size=min(512, max(16, self.settings.embedding_batch_size)),
            ...
            for query in query_vectors:
                hits_returned += len(index.search(query, top_k))
            ...
            metrics = {
                ...
                "measurement_scope": "synthetic_vector_kernel_only",
