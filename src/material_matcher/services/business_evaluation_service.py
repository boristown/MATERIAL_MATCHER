...
class BusinessEvaluationService:
    """Evaluate frozen task results against a labeled business truth file.

    This layer is intentionally read-only with respect to matching results. It
    measures model/rule quality and human-review gain without changing a task.
    """
        ...
        for row in iter_tabular_rows(Path(str(record["stored_path"]))):
            ...
            if not key or not expected:
                raise DomainError(
                    ...
                    "验收文件存在源记录标识或正确集团码为空的行",
                    status_code=422,
                    details={"key_column": key_column, "expected_column": expected_column},
                )
            previous = truth.get(key)
            if previous is not None and previous != expected:
                raise DomainError(
                    "EVALUATION_LABEL_CONFLICT",
                    f"验收文件中标识“{key}”对应了多个不同集团码",
                    status_code=422,
                    details={"key": key, "first": previous, "second": expected},
                )
            truth[key] = expected
        if not truth:
        ...
        *,
        ...
        for key, expected in truth.items():
            ...
            if original_status == "MATCHED":
                automatic_rows += 1
                automatic_correct += int(top1_ok)
            elif original_status == "REVIEW":
                review_rows += 1
            elif original_status == "UNMATCHED":
                unmatched_rows += 1
            if final:
                resolved_rows += 1
        ...
        with self.meta.connect() as connection:
            connection.execute(
                ...
                (run_id, task_id, truth_file_id, key_mode, key_column, expected_group_code_column, json.dumps(metrics, ensure_ascii=False, separators=(",", ":")), created_at),
            )
            connection.executemany(
                "INSERT INTO evaluation_items(run_id,truth_key,expected_group_code,matched_task_row,source_row_id,source_id,original_status,current_status,top1_group_code,final_group_code,top1_score,top1_correct,final_correct,expected_candidate_rank) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                [
                    (
                        run_id,
                        item["truth_key"],
            ...
            connection.execute(
                ...
                (uuid.uuid4().hex, "evaluation", run_id, "BUSINESS_ACCURACY_EVALUATED", json.dumps({"task_id": task_id, "metrics": metrics}, ensure_ascii=False, separators=(",", ":")), created_at),
            )
