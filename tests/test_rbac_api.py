...
def test_viewer_is_read_only_and_operator_can_create_task_draft(authed: TestClient) -> None:
    ...
    authed.post('/api/users/viewer1/reset-password', json={'password': 'Viewer23456', 'must_change_password': False})
    _login(authed, 'viewer1', 'Viewer23456')
    ...
    _login(authed, 'admin', TEST_ADMIN_PASSWORD)
    _create_user(authed, 'operator1', 'operator', 'Operator12345')
    authed.post('/api/users/operator1/reset-password', json={'password': 'Operator23456', 'must_change_password': False})
    _login(authed, 'operator1', 'Operator23456')
...
def test_reviewer_can_review_but_cannot_create_configuration(authed: TestClient) -> None:
    ...
    authed.post('/api/users/reviewer1/reset-password', json={'password': 'Reviewer23456', 'must_change_password': False})
    _login(authed, 'reviewer1', 'Reviewer23456')
...
def test_forced_password_change_allows_only_auth_actions_until_changed(authed: TestClient) -> None:
    ...
    reused = authed.post('/api/auth/change-password', json={'current_password': 'Initial12345', 'new_password': 'Initial12345'})
