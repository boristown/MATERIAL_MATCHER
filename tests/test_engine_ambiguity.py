...
def _config(success: int = 60, tie_break: str = "top1") -> MatchingConfig:
    return MatchingConfig.model_validate({
        ...
        "retrieval": {"mode": "scan"},
        "decision": {"success_threshold": success, "review_enabled": True, "review_threshold": 20, "top_n": 5, "tie_break": tie_break},
    })
...
def test_tie_default_auto_selects_first_candidate(tmp_path: Path) -> None:
    target = _write(tmp_path, "target.csv", "编码,集团码,名称,型号", [
        "T1,G1,电阻,R10", "T2,G2,电阻,R10",
    ])
    source = _write(tmp_path, "source.csv", "编码,名称,型号", ["0001,电阻,R10"])
